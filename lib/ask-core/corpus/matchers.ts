...
export const TWO_PLAYERS = new RegExp(
  `\\b(both|two (players|people|of us)(?! ago)|more than one|same (tile|discard|one|\\d ?(bams?|craks?|dots?)|north|south|east|west|soap|flower|dragon)|at the same time|simultaneous(ly)?|who gets|who has priority|priority|first dibs)\\b|${HOLD_WAIT.source}`,
  "i",
);
...
export const LAST_OF_WALL =
  /\b((?<!(?:my|your|her|his|their|own) )(last|final) (tile|discard|few tiles)|(cold|hot) wall|wall (runs|is|gets) (out|empty|gone|done|low|short|thin|down)|wall is (almost|nearly|about) (gone|empty|done|out)|wall gets (down )?to\b|last \d+ tiles|last (few|couple|handful of) tiles|run(s|ning)? out of tiles|no (more )?tiles? left|end of the (wall|game)|out of tiles|last (one|tile|few) (in|of|on) the wall|last discard of the (game|hand|round)|low wall|wall gets low)\b/i;
...
export const FINAL_DISCARD_SCENE =
  /\b(cold|hot) wall\b|(?<!(?:my|your|her|his|their|own) )\b(last|final) (discard|tile)\b(?=[^.?!]{0,40}\b(wall|deal|game|end|empty|left)\b)|\b(wall|deal|game|end|empty)\b[^.?!]{0,40}(?<!(?:my|your|her|his|their|own) )\b(last|final) (discard|tile)\b|\bwall gets (down )?to\b|\blast \d+ tiles\b|\blast (few|couple|handful of) tiles\b|\bwall is (almost|nearly|about) (gone|empty|done|out)\b|\bwall is (empty|gone|out|used up)\b|\bwall runs out\b|\bno tiles left\b|\bout of tiles\b|\bend of the wall\b|\bnothing left to draw\b|\blast discard of the (game|hand|round)\b|\b(very )?last (one|tile) in the wall\b|\bwall (gets|is|runs) low\b/i;
...
export const OTHER_CLAIMER =
  /\b(another|other|next|someone else|anyone else|the other|any other|somebody else)\s+(player|person|one|lady|guy)?\b[^.?!,;]{0,16}\b(call|calls|called|calling|claim|claims|claimed|take|takes|took|get|gets)\b|\b(she|he|they)\s+(called|claimed|took|takes|calls|claims)\b[^.?!,;]{0,16}\b(it|the tile|my discard|that tile)\b/i;
