...
const CARD_DIGIT = /\b(numbers?|digits?|numerals?)\b|\b[1-9]{1,4}\b|\b(two|three|four|five) \ds\b/i;
...
const RUN_WORD =
  "(runs?(?! out)|sequences?|sequential(ly)?|in (numerical |number |numeric )?order|in a row|one after (the other|another)|consecutively|straight(?! (on|through|along|away|up|out|to|into|off|from)\\b))";
...
const SETTLE_ANY = new RegExp(
  `${PAYMENT.source}|\\b(kicks? in|chips? in|coughs? up|toss(es|ed)? in|throws? in|puts? in|ante|antes|kitty|pot|pennies|penny|cents?|bucks?|quarters?|a dollar|dollars?|nickels?|dimes?|flat (amount|fee|rate)|exchange (nothing|money|anything)|anything change hands|settle up|square up|even up|keep (our|my|their|your) (money|pennies|quarters|chips|cash))\\b`,
  "i",
);
const WALL_GAME_PAY = new RegExp(
  `\\bwall game\\b[^.?!]{0,60}(${SETTLE_ANY.source})|(${SETTLE_ANY.source})[^.?!]{0,60}\\bwall game\\b|(${NO_WINNER})[^?!]{0,60}(${SETTLE_ANY.source})|(${SETTLE_ANY.source})[^?!]{0,60}(${NO_WINNER})`,
  "i",
);
const TALK_VERB =
...
const LOOK_VERB = /\b(look|looks|looking|looked|see|sees|seeing|view|viewing|check|checking|peek|peeking|glance|examine|inspect|read|study|stare|staring|scan)\b/i;
...
export const LVM_CARDSCORING: CanonicalRule[] = [
  {
    ...
    blocks: [/\bwhat does (the )?(little |letter |a |an )?[cx] (mean|stand)\b|\b(letter|letters)\b|\b[cx] (after|next to|beside|behind)\b/i, LIKE_NUMBER, (q: string) => CONSECUTIVE.test(q) && !REPEAT_IN_A_ROW.test(q), OLD_CARD, LEGEND_ASK, JOKER],
    answer:
  ...
  },
  {
    ...
    blocks: [
      REPEAT_IN_A_ROW, BLIND_PASS, CHARLESTON_WORD,
  ...
  },
  {
    ...
    blocks: [/\b(forgot|forget|forgetting|didn'?t (say|announce|declare|call)|failed to (say|announce))\b/i, 
  ...
  },
  {
    ...
    blocks: [/\b(loss cap|cap on losses|caps? (our |the |my )?losses|ante|antes|kitty)\b/i, 
  ...
  },
  {
    ...
    // their own entries.
    blocks: [/\bwall game\b/i, SELF_PICK, JOKER_ANY, DEAD, ERROR_CUE, MISNAMED, TOURNAMENT, LAST_OF_WALL, NO_WINNER_RE, SKIPPED_DRAW],
    answer:
  ...
  },
  {
    ...
    blocks: [JOKER_ANY, ERROR_CUE, TOURNAMENT],
    answer:
  ...
  },
  {
    ...
    blocks: [/\bdice\b|\bwho('s| is) east\b|\bhouse rule\b[^.?!]{0,30}\b(card|league)\b|\b(card|league)\b[^.?!]{0,30}\bhouse rule\b/i, 
      MISNAMED, JOKER, /\bpairs?\b/i, CHARLESTON_WORD, BLIND_PASS, DEAD, QUINT_SEXTET,
