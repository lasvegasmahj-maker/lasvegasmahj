...
`.github/workflows/ci.yml` runs on every pull request to `main` and every push to `main`:
...
The rules corpus and the Ask engine are the shared package mahj-ask-core, vendored under `lib/ask-core/` and pinned by `ask-core.lock.json`. Never edit `lib/ask-core` here; change the core, release, then `node scripts/ask-core/ask-core-sync.mjs v<version>`. The **checks** job runs `scripts/ask-core/ask-core-check.mjs` first, which fails on a modified vendored file or a lock older than Find My Mahj's main. See `docs/ask-rules-knowledge.md`.
